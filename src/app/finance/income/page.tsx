  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">수입재정집계표</h1>
                ...
                return [
                  ...mw.map((w, idx) => (
                    <tr key={`${m}-${w.day}-${idx}`} className="hover:bg-blue-50/50">
                      ...
                      <td className="px-3 py-2 text-center text-gray-600 dark:text-gray-300">{Number(w.day)}</td>
                      {w.cols.map((n, i) => <td key={i} className="px-3 py-2 text-right text-gray-700">{n ? n.toLocaleString() : '-'}</td>)}
                      <td className="px-3 py-2 text-right font-medium text-gray-900 dark:text-gray-100">{w.sumExBuild.toLocaleString()}</td>
